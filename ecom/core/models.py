...
class Order(models.Model):
    ...
    def save(self,*args,**kwargs):
        if self.order_id is None and self.datetime_ofpayment and self.id:
            # self.order_id = self.datetime_ofpayment.strftime('ORDER%Y%m%dODR') + str(self.id)
            self.order_id = self. datetime_ofpayment.strftime('PAY2ME%Y%m%dODR') + str(self.id)  # PAY2ME20220712ODR 101
